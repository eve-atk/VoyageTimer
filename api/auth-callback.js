var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { createToken, verifyToken } from './lib/auth';
function send(res, statusCode, message) {
    res.statusCode = statusCode;
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.end(message);
}
function getAllowedUsers() {
    var _a;
    return ((_a = process.env.ALLOWED_GITHUB_USERS) !== null && _a !== void 0 ? _a : '')
        .split(',')
        .map(function (item) { return item.trim(); })
        .filter(function (item) { return item.length > 0; });
}
export default function handler(req, res) {
    return __awaiter(this, void 0, void 0, function () {
        var clientId, clientSecret, callbackUrl, tokenSecret, requestUrl, code, state, statePayload, redirect, tokenResponse, tokenJson, githubAccessToken, userResponse, user, login, allowedUsers, appToken, redirectUrl;
        var _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    clientId = process.env.GITHUB_CLIENT_ID;
                    clientSecret = process.env.GITHUB_CLIENT_SECRET;
                    callbackUrl = process.env.AUTH_REDIRECT_URI;
                    tokenSecret = process.env.AUTH_JWT_SECRET;
                    if (!clientId || !clientSecret || !callbackUrl || !tokenSecret) {
                        send(res, 503, 'OAuth 環境変数が不足しています。');
                        return [2 /*return*/];
                    }
                    requestUrl = new URL((_a = req.url) !== null && _a !== void 0 ? _a : '/', 'https://placeholder.local');
                    code = requestUrl.searchParams.get('code');
                    state = requestUrl.searchParams.get('state');
                    if (!code || !state) {
                        send(res, 400, 'code または state が不足しています。');
                        return [2 /*return*/];
                    }
                    statePayload = verifyToken(state, tokenSecret);
                    if (!statePayload || statePayload.typ !== 'oauth_state') {
                        send(res, 401, 'state 検証に失敗しました。');
                        return [2 /*return*/];
                    }
                    redirect = statePayload.redirect;
                    if (typeof redirect !== 'string') {
                        send(res, 400, 'state の redirect が不正です。');
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, fetch('https://github.com/login/oauth/access_token', {
                            method: 'POST',
                            headers: {
                                Accept: 'application/json',
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({
                                client_id: clientId,
                                client_secret: clientSecret,
                                code: code,
                                redirect_uri: callbackUrl,
                                state: state,
                            }),
                        })];
                case 1:
                    tokenResponse = _b.sent();
                    if (!tokenResponse.ok) {
                        send(res, 502, 'GitHub トークン交換に失敗しました。');
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, tokenResponse.json()];
                case 2:
                    tokenJson = (_b.sent());
                    githubAccessToken = tokenJson.access_token;
                    if (!githubAccessToken) {
                        send(res, 502, 'GitHub アクセストークンを取得できませんでした。');
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, fetch('https://api.github.com/user', {
                            headers: {
                                Authorization: "Bearer ".concat(githubAccessToken),
                                Accept: 'application/vnd.github+json',
                                'X-GitHub-Api-Version': '2022-11-28',
                            },
                        })];
                case 3:
                    userResponse = _b.sent();
                    if (!userResponse.ok) {
                        send(res, 502, 'GitHub ユーザー情報の取得に失敗しました。');
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, userResponse.json()];
                case 4:
                    user = (_b.sent());
                    login = user.login;
                    if (!login) {
                        send(res, 502, 'GitHub ユーザーIDの取得に失敗しました。');
                        return [2 /*return*/];
                    }
                    allowedUsers = getAllowedUsers();
                    if (allowedUsers.length > 0 && !allowedUsers.includes(login)) {
                        send(res, 403, 'このユーザーは更新権限を持っていません。');
                        return [2 /*return*/];
                    }
                    appToken = createToken({
                        typ: 'access',
                        sub: login,
                        login: login,
                    }, tokenSecret, 60 * 15);
                    redirectUrl = new URL(redirect);
                    redirectUrl.hash = "auth_token=".concat(encodeURIComponent(appToken), "&auth_user=").concat(encodeURIComponent(login));
                    res.statusCode = 302;
                    res.setHeader('Location', redirectUrl.toString());
                    res.end();
                    return [2 /*return*/];
            }
        });
    });
}
